...
# support.php answers a POST with a full page, so it needs the same shell every
# other page has. It is GENERATED here rather than hand-copied into the PHP
# because head_html() cache-busts styles.css by CONTENT HASH -- a hand-written
# copy would keep asking for last month's stylesheet, and nothing would say so.
# One nowdoc, two placeholders, so the PHP never has to quote any of this HTML.
_SHELL_TITLE = "@@NOCTURNE_TITLE@@"
_SHELL_BODY = "@@NOCTURNE_BODY@@"
_SHELL_EOT = "NOCTURNE_SHELL_HTML"


def php_shell_source() -> str:
    """`site/_shell.php`: nocturne_shell($title, $body) rendering the real shell."""
    meta = {"title": _SHELL_TITLE,
            "description": "Report a problem with Nocturne.",
            "scripts": ["main.js"]}
    # Rendered through render() itself, so nav, footer, fonts, metadata and the
    # hashed stylesheet URL are the SAME objects the static pages get.
    page = render("support.html", meta, _SHELL_BODY)
    assert _SHELL_EOT not in page, "nowdoc terminator appears in the page"
    assert page.count(_SHELL_BODY) == 1
    assert page.count(_SHELL_TITLE) >= 1
    return f"""<?php
// GENERATED by packaging/build_site.py -- do not edit by hand; run the builder.
// Keeps support.php's response page identical to every other page on the site.
declare(strict_types=1);

function nocturne_shell(string $title, string $body): string {{
    $page = <<<'{_SHELL_EOT}'
{page}{_SHELL_EOT};
    // $body is trusted markup assembled by the caller; $title is escaped here so
    // a caller can never put an unescaped string into <title> or og:title.
    return str_replace(
        ['{_SHELL_TITLE}', '{_SHELL_BODY}'],
        [htmlspecialchars($title, ENT_QUOTES, 'UTF-8'), $body],
        $page);
}}
"""
...
def build(check: bool = False) -> list[str]:
    ...
    src_php = php_shell_source()
    if not (shell.exists() and shell.read_text() == src_php):
        changed.append("_shell.php")
        if not check:
            shell.write_text(src_php)
    return changed
