"""Generate the Gallery page from a folder of SHARED images.
    ...
    *** site/_src/gallery.html IS GENERATED BY THIS FILE. ***

Not only site/gallery.html — the _src one too, whole, every run. An edit made
there is silently undone the next time this script runs, which is what happened
on 2026-09-21: Andreas's prose-review wording ("no other editor" removed, "a
telescope that costs less than a lens" replaced) went back to the old text, and
a paragraph added by hand vanished. The page's prose lives in page_html() below.
