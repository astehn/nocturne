...
### Added
- Stacking can run in the background. The Stack dialog has a second button, 'Stack in background', beside the one it always had; the stack moves to a separate process and the app stays fully usable while it runs. Outstanding jobs appear in a Jobs panel with their progress, and a finished background stack is logged rather than opened — it never swaps the canvas out from under work you have started. One job runs at a time, which is a memory guarantee rather than a scheduling choice: two 300-frame drizzles at once do not fit. Quitting with jobs running warns, cancels them, and genuinely waits for them to stop. Mosaic and Ha/OIII stacks stay in the foreground for now; the background protocol carries a plain stack only.
- Every pipeline step now says whether its settings have been applied. A step you have adjusted but not committed says so in the panel, and the green Apply button is spent only when there is actually an edit waiting — a colour worn at all times says nothing at the moment it matters. This was the first thing a new user asked about: the preview is pixel-identical to what Apply produces, by design, so nothing on screen distinguished 'looking at it' from 'done'.
- Reset step, on every stage that commits. It puts the controls back to their defaults and removes that step's commit, and it says what it will discard before it does.
- De-green Sky is its own step now, immediately after Stretch, instead of a button on the Color panel five steps before its own cause. The green cast it fixes is CREATED by the stretch — a Bayer sensor gives green twice the photosites, so a linked stretch amplifies it about 1.9x — so the panel's own advice ('reach for this only if a cast survives the stretch') is finally something you can act on where you are standing.
- The zoom pill shows the zoom level as a percentage, always, so 'am I at 100%?' is answerable without counting clicks.
- R, G and B in the pixel readout are shown in their own colours. They are easy to skim past, and they are the numbers that tell you what the picture is actually made of.
- The Share preview pans and zooms. Judging a title plate's placement, colour and size at pane scale meant judging a 4096-pixel image at about a quarter size; you can now zoom in on the plate and keep editing its colour while looking at it.
- Every tool that separates stars says which engine did it — RC-Astro's StarXTerminator or the free detector — in the log, and in the panel where there is one. The two separations differ enough that telling them apart previously took a measurement rather than a glance.

### Changed
- The two green tools are renamed for what they act ON: Remove Green is now De-green Sky, and Remove Green Fringe is now De-green Stars. Projects and recipes saved under the old names still load.
- De-green Stars is a switch rather than a strength, and it selects green by HUE. The old operator removed green wherever green exceeded the red/blue average, which is true of cyan and yellow-green too: measured on two real drizzled masters, only 4.0% and 5.6% of the green it removed came from pixels whose hue actually reads green, while on one of them 49.5% came from CYAN and on the other 35.5% came from YELLOW. It was spending almost all its effort dragging correctly coloured stars toward magenta or orange, which is also why it appeared to do nothing useful. Now only genuinely green pixels move, and they become grey of the same brightness rather than being deleted, so a star keeps its size. It is a switch because no star is green — blackbody colour runs red, orange, yellow, white, blue, and never passes through it — so there is no such thing as wanting to keep 40% of it. Selecting by hue is also what let the star mask go: there is nothing left to protect by aiming.
- The sweeping progress bar over the image is reserved for work you asked for that is about to change the image. Walking into a step and finding it separating stars reports in the right panel alone, with its label, elapsed time, progress and Cancel — it is neither requested nor about to change anything, and two indicators doing the same job read as inconsistency.
- Narrowband and Colour Balance say 'Separating stars' rather than 'Removing stars'. The split keeps both layers, and six other places in the app already said so.
- The Stretch step loses its Target dropdown, and its default is now derived rather than chosen. The dropdown had four entries, three distinct values and two that were identical; "Auto" set the slider to the value it already had, so it did nothing; and the binding was one-way, so after nudging the slider the box went on reading "Nebula" over a value that was no longer Nebula's. The default is now the slider position whose target background matches the one the display preview already uses — so arriving at Stretch and pressing Apply is byte-identical to what was already on screen. The old default landed 8.9% brighter, moving 94.7% of pixels by more than one 8-bit step.
- De-green Stars has no setting at all now; Apply performs it. It is one action with nothing to choose, and a checkbox in front of it was a step performed for the app's benefit.
- The Colour step separates its optional Tint controls behind a rule and an "Optional" heading, and Reset step moves below the tool's own controls behind the same rule on every panel. Reset sat directly under Apply, where it read as part of the tool rather than as the recovery it is; it is now tinted warm to mark it as a different kind of action.

### Fixed
- Apply silently discarded every later step. Six commit paths each computed the same truncation and called it straight away, and only one of them had ever been noticed: applying Levels again after a Curves and a Saturation threw both away with nothing said and no undo. All six now ask, and name what is at risk rather than counting it — '3 steps' does not tell you whether you are about to lose ten minutes of GraXpert.
- Rotate and Flip discarded ALL processing on one toolbar click, with no confirm and no way back. They ask now, and say why everything goes: re-framing the picture invalidates the processing whenever it was applied.
- Navigating to a step after Stretch on a still-linear image silently threw away a toolbar commit. The app stretches on your behalf there, and that truncates — run Narrowband on a linear image, click Levels, and the combine was gone without a word.
- Opening another image, opening or closing a project, and quitting all discarded an unapplied preview in silence. Navigation had asked since this release's step work; nothing else did, because the unsaved-changes guard only knew about committed edits.
- The truncation confirm defaulted to the DESTRUCTIVE button. Qt returns a message box's buttons in layout order rather than the order they were added, so pressing Return — or the reflex of hitting the highlighted button — irreversibly discarded the named steps.
- A revisited process step reported the wrong option. Apply Deconvolution at 'strong', navigate away and come back, and the dropdown read 'medium' over an image with 'strong' baked into it.
- An adjusted crop box was thrown away by Next without asking. A drawn box is uncommitted work like any slider value, but it lives on the canvas rather than in a panel, so nothing was watching it.
- Combine could lose data, and two dialogs writing the same file could race each other.
- Every new control was a fresh chance to leave a button clickable during a long operation; four such omissions were found in this release's own review. The step panel's buttons are now disabled as a rule rather than by a list that goes stale.
- The mosaic explanation vanished when the Stack dialog's help was collapsed, which is exactly when it was needed.
- A stylesheet comment written in the wrong syntax had been silently discarding every style rule after it, so the zoom readout, the new panel dividers and the Reset step tint were all rendering with Qt's defaults rather than Nocturne's.
- A slider moved away and put back where you found it still left the step claiming unapplied changes — the Apply button stayed lit, and moving on demanded a choice between Cancel and Discard over a change that no longer existed. Nine steps did this. A step now compares its controls against both what it committed and what they read untouched, so returning to either clears it.
