...
def test_parameters_skips_scalars_lists_dicts_only_as_configured():
    ...
    # Named, not "values: 0.02, 1.0, 1.0" — the reader had to guess which number
    # was which until 2026-09-17. Names come from the sliders' own labels.
    assert "- black point: 0.02" in params
    assert "- midtones: 1.0" in params
...
def test_footer_uses_passed_version_and_date():
    r = provenance.build_report(_entries(), META, app_version="0.4.0", date=D)
    # The version belongs to the REPORT. The old wording read as "made with
    # 0.4.0", which is false for a project processed under an older build and
    # opened today — see test_the_footer_attributes_the_version_to_the_report.
    assert "Report generated by Nocturne 0.4.0 on 25 July 2026" in r
