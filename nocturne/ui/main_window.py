...
# Measured OUT: saturation at 0.50 (max |Δ| 6.0e-8) and star_reduction at 0
# (2.98e-8, the screen-recombined split) — invisible, but not identity, so
# their Apply stays live. Never here: stretch, green_fringe (100), colour and
# the compute steps, whose defaults do real work.
...
class MainWindow(QMainWindow):
    ...
    def _slot_is_the_commit(self, step_id: str, value) -> bool:
        ...
        The neutral counts ONLY while nothing is committed (consistent panels,
        spec §4). Once the step has a commit, the controls describe a change
        exactly when they differ from it: Recover Core applied at 0.30 and
        dragged back to 0.00 is an edit — pressing Apply would undo the 0.30 —
        and reading it as "back where you found it" hid that from both the
        button and Next's prompt.
        """
        committed = self._committed_option(step_id)
        if committed is None:
            neutral = getattr(self._panel, "neutral_option", None)
            return neutral is not None and _same_option(neutral, value)
    ...
    def _enhance(self, op: str) -> None:
        if self.project is None or self._busy:
            return
        if op in ("Star Colour", "Sharpen Nebulosity"):
            self._enhance_with_star_split(op)
            return
    ...
    def _enhance_with_star_split(self, op: str) -> None:
        """The two taps that need a REAL star/starless split rather than a
        threshold mask, so they share one path.

        Star Colour lifts saturation on the STARS layer; Sharpen Nebulosity
        sharpens the STARLESS one. Both screen the untouched other layer back, so
        each affects exactly one and never the other — which is the entire reason
        sharpening is safe here. Sharpening a stretched frame globally rings the
        stars, and taking them out first removes that failure mode rather than
        warning about it.

        The split is slow, so it runs off the UI thread with a busy panel, and is
        recomputed per tap.
        """
        base = self.project.current()
        ...
        def work():
            ...
            if op == "Sharpen Nebulosity":
                return sharpen_nebulosity_layers(starless, stars)
        ...
        self._run_busy(work, on_result, f"{op}…", f"{op} failed")
    ...
    def _refresh(self) -> None:
        ...
        self._next_btn.setEnabled(has_next and not self._busy)
