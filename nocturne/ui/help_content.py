...
_TOPIC_LIST = (
    ...
    _t("narrowband", "Narrowband",
       ...
       "<p><b>If the result looks washed out, this is the control to move — not Saturation.</b> "
       "&times;1.00 matches oxygen to hydrogen, so wherever both gases are strong the red and "
       "blue channels come out equal and the bright core goes grey. That is the matched point "
       "working as intended, but it is also the LEAST colourful setting on the slider: moving "
       "in either direction adds colour. Measured on a Pacman render, the nebula&rsquo;s colour "
       "roughly triples at &times;0.70 (warm and gold) and rises again by &times;2.00 (cool and "
       "teal), with the minimum sitting just above &times;1.00.</p>"
       "<p>Saturation cannot rescue that core, because saturation scales how far a colour sits "
       "from grey and a neutral pixel sits at zero &mdash; any multiple of nothing is still "
       "nothing. Set the balance you want here first, then use Saturation to taste.</p>"
